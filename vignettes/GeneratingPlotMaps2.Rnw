...
\textit{This is an update of the original GeneratingPlotMaps RNW -- this one positions the plot maps starting at the bottom left corner, so that we can identify it as a subplot and track it in the SORTIE-ND outputs.}
...
<<>>=
 ...
 write.table(crackers[,-c(6)], file="BBBPIPO.txt", sep="\t", row.names=FALSE, quote=F)
 write.table(trinity[,-c(6)], file="CCRPIPO.txt", sep="\t", row.names=FALSE, quote=F)
 write.table(realtor[,-c(6)], file="CRCRPIPO.txt", sep="\t", row.names=FALSE, quote=F)
 write.table(bellow[,-c(6)], file="EMRIDGE.txt", sep="\t", row.names=FALSE, quote=F)
 write.table(reclusive[,-c(6)], file="EMSLOPE.txt", sep="\t", row.names=FALSE, quote=F)
 write.table(octane[,-c(6)], file="FRPIJE.txt", sep="\t", row.names=FALSE, quote=F)
 write.table(sodium[,-c(6)], file="PGABMA.txt", sep="\t", row.names=FALSE, quote=F)
 write.table(distress[,-c(6)], file="POFLABMA.txt", sep="\t", row.names=FALSE, quote=F)
 write.table(gravy[,-c(6)], file="SFTRABMA.txt", sep="\t", row.names=FALSE, quote=F)
 write.table(trigger[,-c(6)], file="SUABCO.txt", sep="\t", row.names=FALSE, quote=F)
 write.table(rigid[,-c(6)], file="SUPILA.txt", sep="\t", row.names=FALSE, quote=F)
@

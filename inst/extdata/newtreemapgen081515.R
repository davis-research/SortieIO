...
for(i in 1:nrow(plotspecies)){
  ...
  diaval <- sample(sortieTrees[sortieTrees$Plot==plotspecies[i, 1] & sortieTrees$Species==plotspecies[i, 2], "Diam"],
                   numrows, replace=T)
  multiplier <- runif(numrows, 0.9, 1.1)
...
for(i in 1:length(justplots)){
  write.table(newdf[newdf$Plot==justplots[i], 1:6], file=paste("081715-", justplots[i], ".txt", sep=""), quote=F, row.names=F, sep="\t")
}

save(newdf, file="081715-newdf.RData")
